...
let rec print_tabs (num_tabs : int) : unit = 
  if num_tabs > 0 then (print_string "  "; print_tabs (num_tabs - 1)) else ()

let rec print_ast (ast: Expr.t) (num_tabs : int) : unit = 
  print_tabs num_tabs;
  match ast with
    | EVar x -> print_string x; print_string " "
    | EInt n -> print_int n; print_string " "
    ...
    | _ ->
      print_endline "Not supported yet."
...
let get_action : unit -> Action.t option = fun _ ->
...
let () =
  interactive_loop (Cursor (parse "if true then (fun x -> x + 1) else (fun x -> 2 * x)")) None

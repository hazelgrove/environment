...
let rec c_to_expr (nodes : node list) (edges : edge list) (root : int) : Expr.t
  ...
  then Expr.tag_to_node tag None None None
    ...
    let get_nth_child n =
      let nth_child =
        List.filter (fun (_, _, child_num) -> child_num = n) adj_nodes
      in
...
let expr_to_c (e : Expr.z_t) : graph * int * int list =
  ...
  let get_var_name (v : Expr.t) : string =
    match v with EVar s -> s | _ -> raise (SyntaxError "")
  in
  ...
  in
  let rec ast_to_c (e : Expr.t) (nodes : node list) (edges : edge list)
      (vars : varlist) : graph * int * varlist =
    let add_subtree (e : Expr.t) (nodes : node list) (edges : edge list)
    ...
    | EPair (e1, e2) ->
        ...
        (add_subtree e2 nodes edges vars root 2, root, vars)
  in
  let rec find_cursor (e : Expr.z_t) (vars : varlist) (index : int) :
      int * varlist =
    let rec extend (vars : varlist) (new_var : Var.t) (index : int) =
      ...
      | hd :: tl ->
          let hd_var, _ = hd in
          if Var.equal hd_var new_var
          then (new_var, index) :: tl
          else hd :: extend tl new_var index
    in
    match e with
    | Cursor _ -> (index, vars)
    ...
    | EIf_C (econd, ethen, _) ->
        find_cursor ethen vars (index + 1 + Expr.size econd)
    | EIf_R (econd, ethen, eelse) ->
        find_cursor eelse vars (index + 1 + Expr.size econd + Expr.size ethen)
    | EFun_L (x, e) ->
        let vars = extend vars x (index + 1) in

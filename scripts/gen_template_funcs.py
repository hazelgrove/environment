...
def parse_args():
    parser = argparse.ArgumentParser(description="generate sboolean functions of n variables.")
    ...
    parser.add_argument('-c','--curriculum', '--generate curiculum to target directory rather than just templates',action='store_true')  # on/off flag
...
def make_nfuncs(n,simplify=False,verbose=False):
    ...
    funcs = list(collapse(bools.espresso_tts(*chunk) for chunk in chunked(ttables,500)))
    print(funcs[:3])
    if simplify: 
...
def prettyPrint(expr,verbose=False): 
    ...
    def pretty_print_helper(input,verbose=True): 
        if input[0] in ['and', 'or']:
            if verbose: print(len(input), input)
            # binop 
            binop = '&&' if  input[0] == 'and' else '||' if input[0] == 'or' else input[0]
            ret_string = pretty_print_helper(input[1],verbose=verbose)
            if verbose: print(ret_string)
...
def make_assert(truth_table): 
    ...
    for rel in truth_table.iter_relation():
        clause = ' '.join(true_false_map[rel[0][ttvar('x',i)]] for i in range(len(rel[0])) )
        clause = f'(f {clause})'
...
def gen_curricula(funcs,ttables):
    ...
    for func,ttable in tqdm(zip(funcs,ttables),total=len(funcs)):
        ...
        test_strings = make_test_strings(ttable_list,map(lambda x: x.to_ocaml(),test_funcs))
...
def save_curriculum(curriculum, targ_dir,max_num_steps,done_action=True): 
    ...
    for i in range(max_start_pos +1):
        if len(curriculum[i]) > 0: 
            ...
            assignment_nums.append(len(curriculum[1]))

    # save config snippet 
    json_str = { 'env':{
        'assignment_dir':os.path.join('/RL_env/data',targ_dir.split('data/')[-1]),
        'num_assignments':len(curriculum),
...
if __name__ == "__main__": 
    ...
    if args.curriculum: 
        curriculum, max_steps = gen_curricula(funcs,ttables)
        save_curriculum(curriculum,args.targ_dir,max_steps)
    else: 
